...
export function parseRemoteDocument(text, baseUrl, requestedKind = 'auto') {
  ...
  if (requestedKind === 'sitemap' || requestedKind?.includes('sitemap')) return parseSitemapXml(text, baseUrl) || { type: 'sitemap', entries: [], children: [] };
  if (requestedKind === 'feed' || requestedKind?.includes('feed')) return parseFeedXml(text, baseUrl) || { type: 'feed', entries: [], children: [] };
...
export function deriveGameName(entry) {
  if (entry.title) {
    const cleaned = stripHtml(entry.title)
      ...
      .replace(/(?:\s*\([^)\r\n]{1,40}\))+\s*/g, ' ')
      .replace(/\s*[|–—-].*$/i, '')
      .replace(/\s+(?:codes|wiki|guide|walkthrough|tier list|trello|answers?)\b.*$/i, '')
